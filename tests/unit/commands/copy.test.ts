...
describe("copy command", () => {
  ...
  describe("argument resolution", () => {
    it("uses sourceArg and destArg when both provided", async () => {
      ...
      expect(mockCopyProviders).toHaveBeenCalled();
      const copyCall = mockCopyProviders.mock.calls[0];
      expect(copyCall[0]).toContain("src");
      expect(copyCall[1]).toContain("dest");
    });
    ...
    it("uses lock source + sourceArg as dest when one arg with lock", async () => {
      ...
      await copy("/my-dest", undefined, defaultOpts);

      const copyCall = mockCopyProviders.mock.calls[0];
      expect(copyCall[0]).toBe("/lock-source");
      expect(copyCall[1]).toContain("my-dest");
    });
    ...
    it("uses lock source + cwd as dest when no args with lock", async () => {
      ...
      await copy(undefined, undefined, defaultOpts);
  ...
  describe("validation", () => {
    it("errors when source does not exist", async () => {
      ...
      await copy("/missing", "/dest", defaultOpts);
    ...
    it("errors when source is not a directory", async () => {
      ...
      await copy("/file", "/dest", defaultOpts);
  ...
  describe("copy execution", () => {
    it("calls copyProviders with force and dryRun options", async () => {
      setupValidSource();
    ...
    it("sets verbose mode from options", async () => {
      ...
      await copy("/src", "/dest", { ...defaultOpts, verbose: true });
  ...
  describe("dry-run output", () => {
    it("displays Would copy and does not call writeLock", async () => {
      setupValidSource();

      await copy("/src", "/dest", { ...defaultOpts, dryRun: true });
  ...
  describe("normal mode", () => {
    it("calls writeLock when items are copied", async () => {
      setupValidSource();

      await copy("/src", "/dest", defaultOpts);

      expect(mockWriteLock).toHaveBeenCalled();
      expect(log.success).toHaveBeenCalledWith("1 item(s) copied");
    });

    it("skips writeLock when nothing copied", async () => {
      mockExists.mockResolvedValue(true);
      mockIsDirectory.mockResolvedValue(true);
      mockFilterProviders.mockReturnValue([claude]);
      mockScanProviders.mockResolvedValue([
